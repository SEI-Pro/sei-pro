...
//	'use strict';
	...
	var fakeSelectedClass = 'cke_table-faked-selection',
		fakeSelectedEditorClass = fakeSelectedClass + '-oEditor',
	...
	function isWidget( element ) {
		// console.log(element);
		return CKEDITOR.plugins.widget && element && (CKEDITOR.plugins.widget.isDomWidgetElement( element ) || CKEDITOR.plugins.widget.isDomWidgetWrapper( element ) );
	}
	...
	function detectLeftMouseButton( evt ) {
		return getMouseButton( evt ) === CKEDITOR.MOUSE_BUTTON_LEFT;
	}
	...
	function getFakeSelectedTable( oEditor ) {
		var selectedCell = oEditor.editable().findOne( '.' + fakeSelectedClass );

		return selectedCell && selectedCell.getAscendant( 'table' );
	}

	function clearFakeCellSelection( oEditor, reset ) {
		var selectedCells = oEditor.editable().find( '.' + fakeSelectedClass ),
			selectedTable = oEditor.editable().findOne( '[data-' + fakeSelectedTableDataAttribute + ']' ),
			i;

		oEditor.fire( 'lockSnapshot' );

		oEditor.editable().removeClass( fakeSelectedEditorClass );
		...
		if ( reset ) {
			...
			if ( isInTable(oEditor.getSelection().getRanges()) ) {
				oEditor.getSelection().reset();
			}
		}
	}

	function fakeSelectCells( oEditor, cells ) {
		var ranges = [],
			range,
			i;

		for ( i = 0; i < cells.length; i++ ) {
			range = oEditor.createRange();
	...
	function restoreFakeSelection( oEditor ) {
		var cells = oEditor.editable().find( '.' + fakeSelectedClass );
		...
		fakeSelectCells( oEditor, cells );
	}

	function fakeSelectByMouse( oEditor, cellOrTable, evt ) {
		var selectedCells = getSelectedCells( oEditor.getSelection( true ) ),
		...
		if ( fakeSelection.active && !fakeSelection.first &&
			( cell || areCellsReallySelected( oEditor.getSelection(), selectedCells ) ) ) {
		...
		if ( cell && fakeSelection.first.getAscendant( 'table' ).equals( cell.getAscendant( 'table' ) ) ) {
			...
			if ( !fakeSelection.dirty && cells.length === 1 && !( isWidget( evt.data.getTarget() ) ) ) {
				return clearFakeCellSelection( oEditor, evt.name === 'mouseup' );
			}
	...
	function fakeSelectionChangeHandler( evt ) {
		var oEditor = evt.editor || evt.sender.editor,
			selection = oEditor && oEditor.getSelection(),
		...
		clearFakeCellSelection( oEditor );

		console.log(isInTable(selection.getRanges()));

		if ( !isInTable(selection.getRanges()) || !selection.isFake ) {
			return;
		}
		...
		// (#2945)
		if ( _getTableElement(ranges[ 0 ], { table: 1 } ).hasAttribute( ignoredTableAttribute ) ) {
			return;
		}

		// In case of whole nested table selection, getSelectedCells returns also
		// cell which contains the table. We should filter it.
		if ( ranges.length === 1 && _getTableElement(ranges[ 0 ]) &&
			_getTableElement(ranges[ 0 ]).is( 'table' ) ) {
			table = _getTableElement(ranges[ 0 ]);
		}
		console.log(_getTableElement(ranges[ 0 ]));
		...
		if ( cells.length > 0 ) {
			oEditor.editable().addClass( fakeSelectedEditorClass );
	...
	function fakeSelectionMouseHandler( evt ) {
		...
		var oEditor = evt.editor || evt.listenerData.oEditor,
			selection = oEditor.getSelection(),
			selectedTable = getFakeSelectedTable( oEditor ),
		...
		function isOutsideTable( node ) {
			return !node.getAscendant( 'table', true ) && node.getDocument().equals( oEditor.document );
		}
		...
		if ( canClearSelection( evt, selection, selectedTable, table ) ) {
			clearFakeCellSelection( oEditor, true );
		}
		...
		if ( !fakeSelection.active && evt.name === 'mousedown' && detectLeftMouseButton( evt ) && table ) {
			...
			// This listener covers case when mouse button is released outside the oEditor.
			CKEDITOR.document.on( 'mouseup', fakeSelectionMouseHandler, null, { oEditor: oEditor } );
		}

		// The separate condition for table handles cases when user starts/stop dragging from/in
		// spacing between cells.
		if ( cell || table ) {
			fakeSelectByMouse( oEditor, cell || table, evt );
		}
	...
	function copyTable( oEditor, isCut ) {
		var selection = oEditor.getSelection(),
			bookmarks = selection.createBookmarks(),
			doc = oEditor.document,
			range = oEditor.createRange(),
			...
			copybinName = ( oEditor.blockless || CKEDITOR.env.ie ) ? 'span' : 'div',
			copybin,
			copybinContainer,
			scrollTop,
			listener;
		...
		copybin.setStyle( oEditor.config.contentsLangDirection == 'ltr' ? 'left' : 'right', '-5000px' );

		copybin.setHtml( oEditor.getSelectedHtml( true ) );

		// Ignore copybin.
		oEditor.fire( 'lockSnapshot' );

		copybinContainer.append( copybin );
		oEditor.editable().append( copybinContainer );

		listener = oEditor.on( 'selectionChange', cancel, null, null, 0 );
	...
	function fakeSelectionCopyCutHandler( evt ) {
		...
		if ( !isInTable(selection.getRanges()) ) {
			return;
		}

		// (#2945)
		if ( _getTableElement(selection.getRanges()[ 0 ], { table: 1 } ).hasAttribute( ignoredTableAttribute ) ) {
			return;
		}


		copyTable( oEditor, evt.name === 'cut' );
	}
	...
	var fakeSelectionPasteHandler = {
		...
		findTableInPastedContent: function( oEditor, dataValue ) {
			var dataProcessor = oEditor.dataProcessor,
				tmpContainer = new CKEDITOR.dom.element( 'body' );

			if ( !dataProcessor ) {
				dataProcessor = new CKEDITOR.htmlDataProcessor( oEditor );
			}
	...
	function pasteListener( evt ) {
		var oEditor = evt.editor,
			...
			boundarySelection = isInTable(selection.getRanges(), true ) && this.isBoundarySelection( selection ),
			pastedTable = this.findTableInPastedContent( oEditor, evt.data.dataValue ),
			tableSel,
			selectedTable,
			selectedTableMap,
			pastedTableMap;
		...
		// content (#520).
		oEditor.once( 'afterPaste', function() {
			...
			fakeSelectCells( oEditor, toSelect );
		} );
		...
		if ( !pastedTable ) {
			...
			oEditor.once( 'afterPaste', function() {
				...
				fakeSelectCells( oEditor, tableSel.cells.all );
				oEditor.fire( 'unlockSnapshot' );
			} );
		...
		setTimeout( function() {
			oEditor.fire( 'afterPaste' );
		}, 0 );

		function isCustomPaste( selection, selectedCells, pastedTable, boundarySelection ) {
			var ranges = selection.getRanges(),
				table = ranges.length && _getTableElement(ranges[ 0 ], { table: 1 } );
	...
	function customizeTableCommand( oEditor, cmds, callback ) {
		oEditor.on( 'beforeCommandExec', function( evt ) {
			if ( CKEDITOR.tools.array.indexOf( cmds, evt.data.name ) !== -1 ) {
				evt.data.selectedCells = getSelectedCells( oEditor.getSelection() );
			}
		} );

		oEditor.on( 'afterCommandExec', function( evt ) {
			if ( CKEDITOR.tools.array.indexOf( cmds, evt.data.name ) !== -1 ) {
				callback( oEditor, evt.data );
	...
	CKEDITOR.plugins.tableselection = {
		...
		/**
		 * Adds keyboard integration for table selection in a given oEditor.
		 *
		 * @param {CKEDITOR.oEditor} oEditor
		 * @private
		 */
		keyboardIntegration: function( oEditor ) {
			// Handle left, up, right, down, delete, backspace and enter keystrokes inside table fake selection.
			function getTableOnKeyDownListener( oEditor ) {
				var keystrokes = {
						37: 1, // Left Arrow
						38: 1, // Up Arrow
						39: 1, // Right Arrow,
				...
				return function( evt ) {
					...
					if ( !keystrokes[ key ] || oEditor.readOnly ) {
						return;
					}

					selection = oEditor.getSelection();

					if ( !selection || !isInTable(selection.getRanges()) || !selection.isFake ) {
						return;
					}

					ranges = selection.getRanges();
					firstCell = _getTableElement(ranges[ 0 ]);
					lastCell = _getTableElement(ranges[ ranges.length - 1 ]);
					...
					if ( key !== 13 || oEditor.plugins.enterkey ) {
					...
					if ( key > 36 && key < 41 ) {
						...
						if ( key === 13 && oEditor.plugins.enterkey ) {
							...
							oEditor.fire( 'lockSnapshot' );
							keystroke === 13 ? oEditor.execCommand( 'enter' ) : oEditor.execCommand( 'shiftEnter' );
							oEditor.fire( 'unlockSnapshot' );
							oEditor.fire( 'saveSnapshot' );
						} else if ( key !== 13 ) {
			...
			function tableKeyPressListener( evt ) {
				...
				if ( !selection || !isInTable(selection.getRanges()) || !selection.isFake || !isCharKey ||
			...
			var editable = oEditor.editable();
			editable.attachListener( editable, 'keydown', getTableOnKeyDownListener( oEditor ), null, null, -1 );
...
function getOnloadTS() {
    ...
    CKEDITOR.document.appendStyleSheet( URL_SPRO + 'js/lib/ckeditor/tableselection.css' );

    console.log('ONLOAD  tableselection');
}
function getInitTS(oEditor) {

    // Disable unsupported browsers.
    console.log('INIT tableselection', oEditor, typeof oEditor);
    if ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) {
        return;
    }
    // Add styles for fake visual selection.
    if ( oEditor.addContentsCss ) {
        oEditor.addContentsCss( URL_SPRO + 'js/lib/ckeditor/tableselection.css' );
    }

    // oEditor.on( 'focus', function() {
        var editable = oEditor.editable(),
            mouseHost = editable.isInline() ? editable : oEditor.document,
            evtInfo = { oEditor: oEditor };

        
        console.log('LOADED tableselection');

        // Explicitly set oEditor as DOM events generated on document does not convey information about it.
        ...
        editable.attachListener( oEditor, 'selectionCheck', fakeSelectionChangeHandler );

        CKEDITOR.plugins.tableselection.keyboardIntegration( oEditor );
    ...
    oEditor.on( 'paste', fakeSelectionPasteHandler.onPaste, fakeSelectionPasteHandler );

    customizeTableCommand( oEditor, [
        'rowInsertBefore',
        'rowInsertAfter',
        'columnInsertBefore',
        'columnInsertAfter',
        'cellInsertBefore',
        'cellInsertAfter'
    ], function( oEditor, data ) {
        fakeSelectCells( oEditor, data.selectedCells );
    } );
    ...
    customizeTableCommand( oEditor, [
        ...
        fakeSelectCells( oEditor, [ data.commandData.cell ] );
    } );
...
function isInTable(ranges, allowPartially = false) {
	if ( ranges.length === 0 ) {
		return false;
	}
	...
	function isPartiallySelected( range ) {
		var startCell = range.startContainer.getAscendant( { td: 1, th: 1 }, true ),
			...
			console.log('isInTable', startCell, endCell);
	...
	for ( i = 0; i < ranges.length; i++ ) {
		node = _getTableElement(ranges[ i ]);
...
function getMouseButton( evt ) {
	...
	return normalizeMouseButton( domEvent.button );
}
function normalizeMouseButton( button, reverse ) {
...
function _getTableElement(this_, tableElements ) {
	...
	var start = this_.getTouchedStartNode(),
		end = this_.getTouchedEndNode(),
	...
	if ( startTable && !this_.root.contains( startTable ) ) {
		return null;
	}

	if ( this_.getEnclosedNode() ) {
		return this_.getEnclosedNode().getAscendant( tableElements, true );
	}
