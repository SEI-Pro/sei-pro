 ...
 */
import { listSchemaEntriesForOptionsSection } from '../config/schema.js';
...
import {
    ...
    loadLlmAiSettings,
    loadLlmProfiles,
    migrateDataValuesSecrets,
    readTextFile,
    requestProfileHostPermissions,
    saveBugReportOptIn,
    saveDataValuesSeparatingSecrets,
    saveLlmProfile,
    saveLlmAiSettings,
...
function renderSchemaOptionsSection(sectionId) {
    const host = document.getElementById('options-schema-' + sectionId);
    if (!host) return;
    host.textContent = '';
    listSchemaEntriesForOptionsSection(sectionId).forEach(({ key, entry }) => {
        if (entry.type !== 'boolean') return;
        const tr = document.createElement('tr');
        const tdLabel = document.createElement('td');
        const p = document.createElement('p');
        const icon = document.createElement('i');
        icon.className = 'iconPopup fa fa-shield-alt azulColor';
        const info = document.createElement('span');
        info.className = 'info';
        info.textContent = entry.label || key;
        p.append(icon, info);
        tdLabel.appendChild(p);
        const tdSwitch = document.createElement('td');
        const wrap = document.createElement('div');
        wrap.className = 'onoffswitch';
        const input = document.createElement('input');
        input.type = 'checkbox';
        input.name = 'onoffswitch';
        input.className = 'onoffswitch-checkbox';
        input.id = 'itemConfigGeral_' + key;
        input.setAttribute('data-name', key);
        input.setAttribute('data-storage', entry.storage || 'configGeral');
        input.tabIndex = 0;
        if (entry.default === true) input.checked = true;
        const label = document.createElement('label');
        label.className = 'onoff-switch-label';
        label.setAttribute('for', input.id);
        wrap.append(input, label);
        tdSwitch.appendChild(wrap);
        tr.append(tdLabel, tdSwitch);
        host.appendChild(tr);
    });
}

function readSchemaLocalSwitch(name) {
...
async function saveOptions(reload) {
    ...
    const configGeral = collectConfigGeral().filter((item) => {
        // Local-only schema keys (e.g. bugReportOptIn) must not enter configGeral.
        const el = document.getElementById('itemConfigGeral_' + item.name);
        return !(el && el.getAttribute('data-storage') === 'local');
    });
    ...
    try {
        ...
        await Promise.all([
            ...
            saveBugReportOptIn(readSchemaLocalSwitch('bugReportOptIn'))
        ]);
        loadedAiSettings = aiSettings;
        // Credentials go to storage.local; sync keeps only non-secret fields (ADR-0015).
        await saveDataValuesSeparatingSecrets(payload);
        ...
        if (reload === true) {
            ...
            // Export includes secrets so a local backup remains restorable.
            downloadJsonFile('config.json', serializeDataValues(payload));
            location.reload();
        }
    } catch (error) {
...
async function loadFile() {
    ...
    try {
        ...
        await saveDataValuesSeparatingSecrets(Array.isArray(parsed) ? parsed : []);
...
async function restoreOptions() {
    const [migrated, aiProfiles, accessAudit, aiSettings, bugReportOptIn] = await Promise.all([
        migrateDataValuesSecrets(),
    ...
    const dataValues = Array.isArray(migrated.dataValues) ? migrated.dataValues : [];
    ...
    const bugReportInput = document.getElementById('itemConfigGeral_bugReportOptIn');
    if (bugReportInput) {
        bugReportInput.checked = bugReportOptIn === true;
        setIconTone(bugReportInput, bugReportOptIn === true);
    }
...
export async function installOptionsPage() {
    renderSchemaOptionsSection('privacy');
    bindEvents();
