...
/** Raiz de composição do contexto VISUALIZAÇÃO — ADR-0005. */
import { createStorage } from '../platform/storage.js';
import { createLogger } from '../platform/logger.js';
import { createMessaging } from '../platform/messaging.js';
import { registerVisualizacaoExclusiveFeatures } from '../generated/visualizacao-feature-registry.js';
import { boot } from '../app/boot.js';
import { ready } from '../dom/index.js';

const ns = globalThis.SeiPro || {};
registerVisualizacaoExclusiveFeatures();

ready(async function () {
    const logger = createLogger({ scope: 'visualizacao' });
    const messaging = createMessaging();
    const storage = createStorage({ messaging });
    const result = await boot('visualizacao', {
        logger,
        messaging,
        storage,
