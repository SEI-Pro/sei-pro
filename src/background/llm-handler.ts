...
// MV3 still requires host permission for service-worker fetches. Known
// providers are optional_host_permissions; custom origins are requested at
// runtime from the options/AI UI (ADR-0015 — no https://*/* wildcard).

/** True when the profile targets only a local Ollama / loopback endpoint. */
export function isLocalLlmProfile(profile = {}) {
    if (String(profile.providerId || '').toLowerCase() === 'ollama') return true;
    const parsed = parseProfileUrl(profile.baseUrl);
...
function readExternalProvidersAllowed(browserApi) {
    return new Promise(function (resolve) {
        ...
        function finish(items) {
            try {
                const raw = items && items.dataValues;
                ...
                for (let i = 0; i < parsed.length; i++) {
                    const configGeral = parsed[i] && parsed[i].configGeral;
                    if (!Array.isArray(configGeral)) continue;
                    for (let j = 0; j < configGeral.length; j++) {
                        if (configGeral[j] && configGeral[j].name === 'llmProvedoresExternos') {
                            resolve(configGeral[j].value !== false);
                            return;
                        }
                    }
                }
            } catch (_) { /* ignore: malformed config → default open (ADR-0015) */ }
            // Default true (ADR-0015 / S.8): open with warning.
            resolve(true);
        }
        try {
            const result = storage.get('dataValues', finish);
            if (result && typeof result.then === 'function') result.then(finish, function () { resolve(true); });
        } catch (_) {
            resolve(true); // ignore: storage.sync unavailable → default open
        }
    });
}
...
export async function resolveLlmRequest(request, browserApi) {
    ...
    if (!isLocalLlmProfile(profileForPolicy)) {
        const externalAllowed = await readExternalProvidersAllowed(browserApi);
        if (!externalAllowed) {
            throw new Error(
                'Provedores de IA externos estão desabilitados pela configuração (llmProvedoresExternos). Use Ollama ou um endpoint local.'
            );
        }
    }
