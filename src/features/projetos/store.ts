 ...
 */
import { migrateLocalFeatureStore, PROJETOS_STORE_VERSION } from '../../config/migrations/index.js';
...
export function getStoreProjetos() {
    ...
    if (parsed && Array.isArray(parsed.projetos)) {
        ...
        const migrated = migrateLocalFeatureStore(base, 'projetos', defaultStore);
        storeState = migrated.store;
        if (migrated.changed) {
            storeState.version = PROJETOS_STORE_VERSION;
            return persistStoreProjetos(storeState);
        }
    } else {
        storeState = defaultStore();
        if (typeof storeState.version !== 'number') storeState.version = PROJETOS_STORE_VERSION;
    }
    storeLastRaw = raw;
    return storeState;
}
