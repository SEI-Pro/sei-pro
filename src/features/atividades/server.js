...
/**
 * Atividades — servidor e autorização de chamadas.
 *
 * Gate/payload puros em domain.js; transporte em io.js. O roteamento da resposta
 * ainda vive aqui (callbacks legados) até cada ramo receber deps explícitas.
 */
import './runtime.js';
import {
...
import { getAtividadesServerUrl, postAtividadesServer } from './io.js';

export function getServerAtividades(param, mode) {
    const urlServer = getAtividadesServerUrl();
    if (urlServer && userHashAtiv != '' &&
        isAtividadesServerModeAllowed(mode, {
            ...
            checkLoadingButtonConfirm: typeof checkLoadingButtonConfirm === 'function'
                ? checkLoadingButtonConfirm
                : () => false
        })
    ) {
        param = buildAtividadesRequestParams(param, mode, {
            userHashAtiv,
            version: typeof VERSION_SPRO !== 'undefined' ? VERSION_SPRO : '',
            getOptionsPro: typeof getOptionsPro === 'function' ? getOptionsPro : () => undefined,
        ...
        if (mode.indexOf('config_update_') !== -1) callAtiv('loadingTagConfig',param.type, 'get');

        var authToken = undefined;

        postAtividadesServer(urlServer, param, { beforeSend: authToken }).then(function (ativData) {
