...
function renderSingleMap(id_procedimento, readonly) {
    ...
    function locate() {
        ...
        const html = '<div class="loadingLocation" style="color:#888;position:absolute;z-index:9999;right:0;padding:5px 15px 5px 10px;background:#fff;border-bottom-left-radius:5px;font-size:10pt;">'
            ...
            + '<i class="fas fa-times-circle" data-act="map-clear-location" style="cursor:pointer"></i></div>';
...
// ---- Modal vanilla (substitui jQuery UI dialog para os mapas) ----
function openModal({ title, contentHtml, width, buttons, onOpen, onClose }) {
    ...
    const overlay = elFromHtml(
        '<div class="seipro-monitorado-modal" style="position:fixed;inset:0;background:rgba(0,0,0,.35);z-index:100000;display:flex;align-items:center;justify-content:center;">'
        + '<div class="dialogBoxDiv" style="background:#fff;border-radius:6px;box-shadow:0 8px 30px rgba(0,0,0,.3);max-width:95vw;max-height:95vh;overflow:auto;width:' + (width || 620) + 'px;">'
        + '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid #eee;font-weight:bold;">'
        + '<span>' + title + '</span><i class="fas fa-times" data-act="map-close" style="cursor:pointer;color:#888;"></i></div>'
        + '<div class="seipro-modal-body" style="padding:14px;">' + contentHtml + '</div>'
    ...
    const close = () => { if (typeof onClose === 'function') onClose(); overlay.remove(); };
...
export function openBoxSingleMap(triggerEl, readonly = false) {
    ...
    const buttons = readonly ? [{ text: 'Fechar', onClick: (close) => close() }] : [
        { text: 'Remover', onClick: (close) => { saveConfigMapsMonitorado(id, 'remove'); close(); } },
        { text: 'Salvar', class: 'confirm', onClick: (close) => { saveConfigMapsMonitorado(id); close(); } }
    ];
    openModal({
