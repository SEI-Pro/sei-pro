...
export function refreshSeiPageSelectors(g = globalThis) {
    ...
    const supports = g.SeiPro.sei.supports;
    const selectorsApi = g.SeiPro.sei.selectors;
    ...
    const sidebar = supports ? supports.sidebarLayout() : adapter.isNewSEI();
    const sei5 = supports ? supports.sei5Editor() : adapter.isSEI5();

    g.isNewSEI = isNew;
    g.isSEI_5 = sei5;

    const sel = selectorsApi && typeof selectorsApi.resolve === 'function'
        ? selectorsApi.resolve(isNew, version)
        : adapter.selectors(isNew, version);

    const unitSelNovo = selectorsApi && selectorsApi.UNIT_CONTROL && selectorsApi.UNIT_CONTROL.novo;
    const unitSel = sidebar
        ? unitSelNovo
        : (selectorsApi && selectorsApi.UNIT_CONTROL && selectorsApi.UNIT_CONTROL.legado);
    const institutionSel = sidebar
        ? (selectorsApi && selectorsApi.INSTITUTION_LABEL && selectorsApi.INSTITUTION_LABEL.novo)
        : (selectorsApi && selectorsApi.INSTITUTION_LABEL && selectorsApi.INSTITUTION_LABEL.legado);

    const lnkInfraUnidade = unitSelNovo ? $(unitSelNovo).attr('onclick') : null;
    ...
    g.siglaUnidadeAtual = unitSel
        ? (sidebar ? $(unitSel).text().trim() : $(unitSel).find('option:selected').text().trim())
        : '';
    g.frmEditor = $(sel.frmEditor);
    g.idUnidade = sidebar ? g.infra_unidade_atual : (unitSel ? $(unitSel).val() : null);
    g.divInformacao = sel.divInformacao;
    g.mainMenu = sel.mainMenu;
    g.ancoraArvoreDownload = sel.ancoraArvoreDownload;
    g.infraBarraComandos = sel.infraBarraComandos;
    g.idMenu = sel.idMenu;
    g.infraBarraS = sel.infraBarraS;
    g.nameDocInterno = sel.nameDocInterno;
    g.nomeInstituicao = institutionSel
        ? (sidebar ? $(institutionSel).eq(0).text().trim() : $(institutionSel).text().trim())
        : '';
    g.divComandos = sel.divComandos;
    g.ifrVisualizacao_ = sel.ifrVisualizacao_;
    g['$ifrVisualizacao'] = sel.$ifrVisualizacao;
    g.ifrArvoreHtml_ = sel.ifrArvoreHtml_;
    g['$ifrArvoreHtml'] = sel.$ifrArvoreHtml;
    ...
    try {
        if ($('#sheetsCompleteEtapaForm').length == 0) {
            const host = selectorsApi && selectorsApi.SYSTEM_BAR_HOST
                ? (sidebar ? selectorsApi.SYSTEM_BAR_HOST.novo : selectorsApi.SYSTEM_BAR_HOST.legado)
                : null;
            if (host) $(host).append(g.html_initContentPro);
        }
    } catch (e) { /* ignore */ }
