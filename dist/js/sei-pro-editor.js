(() => {
  ...
  function hydrateQrCodePlaceholders(root3 = globalThis.document, options = {}) {
    if (!root3?.querySelectorAll) return Promise.resolve([]);
    const placeholders = Array.from(root3.querySelectorAll("[data-seipro-qr-code]"));
    return Promise.all(placeholders.map((placeholder) => {
      ...
      return renderQrCode(placeholder, { ...options, text }).catch(() => placeholder);
    }));
  }
  ...
  var Q = class _Q {
    ...
    qrcode(options = {}) {
      ...
      const pending = renderQrCode(el, options).then((result) => {
...
`).replace(/[ \t]+\n/g, "\n"));
  ...
  function setQrCode(qrCode_text) {
    ...
    if (!imgBase && result && result.__seiproQrPromise) {
      result.__seiproQrPromise.then(function() {
        setQrCode(qrCode_text);
      });
      return;
    }
    if (!imgBase) return;
  ...
  function openImageCrop({ src, onSave, onCancel } = {}) {
    ...
    function values() {
      ...
      const sx = clamp2(numberValue(x.input), 0, Math.max(0, sourceWidth - 1));
      const sy = clamp2(numberValue(y.input), 0, Math.max(0, sourceHeight - 1));
      const sw = clamp2(numberValue(cropWidth.input), 1, Math.max(1, sourceWidth - sx));
      const sh = clamp2(numberValue(cropHeight.input), 1, Math.max(1, sourceHeight - sy));
      return {
        x: sx,
        y: sy,
        width: sw,
        ...
        quality: clamp2(numberValue(quality.input), 0.1, 1),
