// Mapas docsLote_specialChars / docsLote_normalChars_utf8 / docsLote_normalChars_iso
// migrados para src/core/docslote.js (Fase 6); globais via aliasGlobal.
...
var docsLote_execute = async (param) => {
    ...
    if (!param.createNewProcs && !getUrlNewDocArvore()) {
        ...
        if (!SeiPro.sei.adapter.isNewSEI()) {
            ...
            const hasSpecialChars = CSVData.some(data => SeiPro.core.docslote.hasDocsLoteSpecialChars(data[param.docsNames], CSVEnconding))
            if (hasSpecialChars) {
